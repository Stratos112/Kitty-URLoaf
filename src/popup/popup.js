...
async function generateCSS() {
  ...
  return [
    ...
    ``,
    `/* ── A: TabsToolbar · paints in the nav-bar zone ─────────── */`,
    block('#TabsToolbar', 'right 150px center', ['  min-height: 150px !important;']),
    ``,
    `/* ── B: nav-bar · paints in the tab-strip zone ────────────── */`,
    `/*   nav-bar is a separate XUL element — testing whether it    */`,
    `/*   composites above the tab strip in Firefox's z-order.      */`,
    block('#nav-bar', 'left 90px center', ['  min-height: 120px !important;']),
    ``,
    `/* give toolbox room for both A and B at full height */`,
