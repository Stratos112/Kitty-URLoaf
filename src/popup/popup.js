...
async function generateCSS() {
  ...
  return [
    ...
    ``,
    `/* ── B: tab bar · above the tabs via ::after ───────────── */`,
    `/*   background renders behind children, so we use ::after  */`,
    `/*   which stacks above the tab elements.                   */`,
    `#TabsToolbar {`,
    `  position: relative;`,
    ...
    `  background-size:     ${sizes};`,
    `  background-repeat:   ${rpts};`,
    `  background-position: ${Array(N).fill('0 0').join(', ')};`,
    `  pointer-events: none;`,
    `  z-index: 9999;`,
    `}`,
    ``,
    ...
    ``,
    `/* ── D: sidebar · targets both old and new Firefox ──────── */`,
    `#sidebar-box,`,
    `#sidebar-main,`,
    `.sidebar-panel {`,
    `  min-width: 2000px !important;`,
    `}`,
    block('#sidebar-box',  'left 10px bottom 10px'),
