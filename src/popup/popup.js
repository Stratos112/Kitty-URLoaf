...
async function generateCSS() {
  ...
  return [
    ...
    ``,
    `/* ── B: tab bar · on the toolbar spacer to the right ────── */`,
    `/*   XUL doesn't support ::after, and background always     */`,
    `/*   paints behind children. We target the toolbar spacer   */`,
    `/*   that sits between the tabs and window controls —       */`,
    `/*   it's a sibling of the tab strip, not a parent.        */`,
    block('.titlebar-spacer[type="post-tabs"]', 'left 0 center',
      ['  min-height: 150px !important;']),
    ``,
    `/* ── C: tab bar · far left on the tab strip ────────────── */`,
    block('#tabbrowser-tabs', 'left 5px center'),
    ``,
    `/* ── D: sidebar · force width with flex override ─────────  */`,
    `#sidebar-box {`,
    `  flex: none !important;`,
