...
HEAD_PRELOAD_S    = 0.5   # head2 transition frame cycling duration (30 frames × 16.7ms = 1 per 60fps refresh)
...
HEAD2_DELAY_S     = 0.7   # stage 4 preload: visible from 0.7s–1.2s = 500ms = all 30 frames at 60fps
...
def head_preload2_keyframes(pos, preload_pos):
    ...
    lines.append(f"  {round(HEAD_PRELOAD_S / total * 100, 1)}% {{ background-image: {sleep_head_imgs}; background-position: {pos}; animation-timing-function: steps(1, end); }}")
