...
css = "\n".join([
    ...
    f"/* pants-ear stays in both hover and active rules so it never restarts (no disappearing ears) */",
    f"/* ear-flick is added on top; later in animation list = higher priority for background-image */",
    f"#nav-bar:hover::after  {{ animation: {anim('pants-ear')}, ear-flick 275ms 200ms linear 1 forwards; background-position: {POS}; }}",
    f"#nav-bar:has(:active)::after {{ animation: {anim('pants-ear')}, ear-flick 275ms linear 1 forwards; background-position: {POS}; }}",
